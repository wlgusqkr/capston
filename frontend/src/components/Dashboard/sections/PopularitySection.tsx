...
//
// Widgets:
//   1. 서울 자취 동 TOP 10 — DongScore[] (score desc already)
//   2. 학교별 자취 TOP 5 — KERNEL_SCHOOL_OPTIONS dropdown (score fallback)
//   3. 인근 비슷한 동 — DongDetail.similar_dongs (top 3)
//
...
export default function PopularitySection({
  ...
  // School-specific data not yet available — fallback to composite score TOP 5.
  const schoolTop5 = (allDongs ?? []).slice(0, 5);
    ...
    <div className="grid grid-cols-3 gap-5">
              ...
              return (
                <li key={d.slug}>
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-card text-left transition-colors duration-[120ms] ease-out hover:bg-primary-soft focus-visible:outline-2 focus-visible:outline-focus-ring focus-visible:outline-offset-2 ${
                      isCurrent ? 'bg-primary-soft' : 'bg-transparent'
                    }`}
                    aria-current={isCurrent ? 'true' : undefined}
                  >
                    <span
                      className={`tabular shrink-0 w-6 text-center text-feature-heading font-semibold ${
                    ...
                    <span className="flex-1 min-w-0">
                      <span className="block text-body-base font-medium text-text truncate">
                        {d.name}
                      </span>
                      <span className="block text-caption text-text-subtle truncate">
                        {d.gu}
                      </span>
                    </span>
      ...
      <Card padding="lg">
        ...
        {schoolTop5.length > 0 ? (
          ...
          <div className="flex items-center justify-center h-[180px] text-text-muted text-caption">
            데이터를 불러오는 중...
          </div>
        )}
        <p className="m-0 mt-3 text-caption text-text-subtle">
          현재는 종합 점수 기준 (학교별 데이터 준비 중)
        </p>
      </Card>
        ...
        {similarTop3.length > 0 ? (
          <div className="flex flex-col gap-3">
            {similarTop3.map((d) => (
              <div
                key={d.slug}
                className="relative rounded-card border border-divider bg-surface p-3 flex flex-col gap-2 transition-colors duration-[120ms] ease-out hover:border-primary"
              >
                <button
                  type="button"
                  onClick={() => onDongSelect(d.slug)}
                ...
                <div className="flex items-start justify-between gap-2 pointer-events-none">
                  <div className="min-w-0">
                    <p className="m-0 text-body-base font-medium text-text truncate">
                      {d.name}
                    </p>
                    <p className="m-0 text-caption text-text-subtle truncate">
                      {d.gu}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-end">
                  <Link
                    ...
                    className="relative z-10 text-caption text-link hover:underline focus-visible:outline-2 focus-visible:outline-focus-ring focus-visible:outline-offset-2 rounded-xs"
