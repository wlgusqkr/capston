"""[TEMP] DP_DB ksci_category -> SLGI 임시 ETL.

단계 6에서 1회 실행. 추후 실 초기 적재 ETL(별도 plan)로 대체 필요.

매핑: ksci_category (ksci_code PK) 1:1.
...
SELECT_SQL = """
SELECT ksci_code, main_category_name, middle_category_name,
       subcategory_name, class_name, subclass_name
FROM ksci_category
"""

INSERT_SQL = """
INSERT INTO ksci_category
  (ksci_code, main_category_name, middle_category_name,
   subcategory_name, class_name, subclass_name)
VALUES (%s, %s, %s, %s, %s, %s)
...
def main() -> int:
    with connect_dp_db() as src, connect_slgi() as dst:
        ...
        print(f"[08_ksci_category] inserted/upserted {n} rows")
        verify_count(src, dst, "ksci_category")
    return 0
