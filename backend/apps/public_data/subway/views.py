...
이동 이력:
- sub-plan 2H: `apps.public_data.transit.views` → `apps.public_data.subway.views`로 이동.
  지하철·버스 혼합 view지만 personality 산정 시 subway 우선이며, NearestSubway가
  주 신호이므로 subway 앱에 둔다. 버스 모델은 apps.public_data.bus에서 import.
"""
